...
const Index = () => {
    const { calculators = [] } = usePage().props; // Ensure toyotasures is an array
    ...
    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete this Data?")) {
            Inertia.delete(route("calculator.destroy", id));
        }
    };
                ...
                <div className="overflow-x-auto shadow-md rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {["#","service","description","history","calculatorservice", "Image", "Actions"].map((header) => (
                                    <th
                                        key={header}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {calculators.map((calculator) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(calculator.service, calculator.id, 'service')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(calculator.description, calculator.id, 'description')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(calculator.history, calculator.id, 'histroy')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(calculator.calculatorservice, calculator.id, 'calculatorservice')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <img
                                            src={`/storage/uploads/Calculator/${calculator.image}`}
                                            alt="Image"
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <Link
                                        href={route("calculator.edit", calculator.id)}
