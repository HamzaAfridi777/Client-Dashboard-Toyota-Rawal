...
const Index = () => {
    const { chooses = [] } = usePage().props; // Ensure toyotasures is an array
    ...
    const handleDelete = (id) => {
        if (confirm("Are you sure you want to delete this Data?")) {
            Inertia.delete(route("choose.destroy", id));
        }
    };
                ...
                <div className="overflow-x-auto shadow-md rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                {["#","Heading", "Description","'Years',","Technicians","Customer",'Employee',  "Actions"].map((header) => (
                                    <th
                                        key={header}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {chooses.map((choose) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.heading, choose.id, 'heading1')}                               
                                    </td>
                                     <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.description, choose.id, 'description')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.years, choose.id, 'years')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.technicians, choose.id, 'technicians')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.customer, choose.id, 'customer')}                               
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {renderTruncatedContent(choose.employee, choose.id, 'employee')}                               
                                    </td>
                                                                 
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <Link
                                        href={route("choose.edit", choose.id)}
                                        ...
                                        <button
                                            ...
                                            onClick={() => handleDelete(choose.id)}
                                        >
